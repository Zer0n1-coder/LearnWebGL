import { gl, loadTexture,SCR_WIDTH,SCR_HEIGHT, clamp } from "./Global"
...
function renderSphere(){
    if(sphereVAO === undefined){
        sphereVAO = <WebGLVertexArrayObject>gl.createVertexArray();

        let vbo = <WebGLBuffer>gl.createBuffer();
        let ebo = <WebGLBuffer>gl.createBuffer();

        let positions = new Array<number[]>();
        let uv = new Array<number[]>();
...
async function hello(what:string) {
    return "ref";
}