...
export function main(): void {
    ...
    function sortDistance(a: number[], b: number[]) {
        let aDis = vec3.length(vec3.fromValues(camera.Position[0] - a[0],
            camera.Position[1] - a[1],
            camera.Position[2] - a[2]));

        let bDis = vec3.length(vec3.fromValues(camera.Position[0] - b[0],
            camera.Position[1] - b[1],
            camera.Position[2] - b[2]));
