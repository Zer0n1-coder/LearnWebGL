...
function keyDown(ev) {
    ...
    if (ev.key === 'w') {
        vec3.add(cameraPos, cameraPos, vec3.fromValues(cameraSpeed * cameraFront[0], cameraSpeed * cameraFront[1], cameraSpeed * cameraFront[2]));
    }
    else if (ev.key === 's') {
        vec3.subtract(cameraPos, cameraPos, vec3.fromValues(cameraSpeed * cameraFront[0], cameraSpeed * cameraFront[1], cameraSpeed * cameraFront[2]));
    }
    else if (ev.key === 'a') {
        let tmp = vec3.create();
        let direction = vec3.normalize(tmp, vec3.cross(tmp, cameraFront, cameraUp));
        vec3.subtract(cameraPos, cameraPos, vec3.fromValues(cameraSpeed * direction[0], cameraSpeed * direction[1], cameraSpeed * direction[2]));
    }
    else if (ev.key === 'd') {
        let tmp = vec3.create();
        let direction = vec3.normalize(tmp, vec3.cross(tmp, cameraFront, cameraUp));
        vec3.add(cameraPos, cameraPos, vec3.fromValues(cameraSpeed * direction[0], cameraSpeed * direction[1], cameraSpeed * direction[2]));
    }
}
export function main() {
    ...
    function render() {
        ...
        mat4.lookAt(view, cameraPos, vec3.add(vec3.create(), cameraPos, cameraFront), cameraUp);
