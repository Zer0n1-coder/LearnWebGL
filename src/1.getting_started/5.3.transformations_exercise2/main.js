...
import { vec3 } from "./gl-matrix/index.js"; //不清楚本地起服务之后ts无后缀导入方式没法定位文件的原因，所以采用.js后缀
export function main() {
    ...
    function render() {
        ...
        mat4.translate(mat1, mat1, [0.5, -0.5, 0.0]);
        mat4.rotateZ(mat1, mat1, curTime);
        ourShader.setMat4('transform', mat1);
        gles.bindVertexArrayOES(VAO);
        gl.drawElements(gl.TRIANGLES, 6, gl.UNSIGNED_SHORT, 0);
        let vec = vec3.create();
        let tmp = Math.sin(curTime);
        let scale = tmp > 0 ? tmp : -tmp; //防止为负数，教程中的参考答案没有处理
        vec3.set(vec, scale, scale, scale);
        let mat2 = mat4.create();
        mat4.translate(mat2, mat2, [-0.5, 0.5, 0.0]);
        mat4.scale(mat2, mat2, vec);
        ourShader.setMat4('transform', mat2);
        gl.drawElements(gl.TRIANGLES, 6, gl.UNSIGNED_SHORT, 0);
